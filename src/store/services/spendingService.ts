...
export const spendingApi = createApi({
	...
	endpoints: builder => ({
		...
		>({
			...
			providesTags: (_result, _error, { spendingType }) => [spendingType],
		}),
		createSpending: builder.mutation<ISpending, { data: Partial<ISpending> }>({
			...
			invalidatesTags: (_result, _error, { data }) => [data.type],
		}),
		...
		deleteSpending: builder.mutation<{ message: string }, number>({
			...
			invalidatesTags: (_result, _error) => [SpendingType.EXPENSE, SpendingType.INCOME],
		}),
